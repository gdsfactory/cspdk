...
from cspdk.si220 import PDK as si220_PDK
...
SI220_EXPECTED_PIN_NAMES: dict[str, set[str]] = {
    "bend_metal": {"e1", "e2"},
    "bend_s_metal": {"e1", "e2"},
    "pad": {"pad"},
...
SI220_CELLS = [pytest.param(n, id=n) for n in SI220_EXPECTED_PIN_NAMES]
...
@pytest.mark.parametrize("cell_name", SI220_CELLS)
def test_si220_logical_pin_registered(cell_name: str) -> None:
    """Si220 cell has logical pins."""
    si220_PDK.activate()
    c = si220_PDK.cells[cell_name]()
    assert c.pins, f"{cell_name} should have logical pins"


@pytest.mark.parametrize("cell_name", SI220_CELLS)
def test_si220_pin_type_is_dc(cell_name: str) -> None:
    """Si220 pin type is DC."""
    si220_PDK.activate()
    c = si220_PDK.cells[cell_name]()
    for pin in c.pins:
...
@pytest.mark.parametrize("cell_name", SI220_CELLS)
def test_si220_expected_pin_names(cell_name: str) -> None:
    """Si220 cell has expected pin names."""
    si220_PDK.activate()
    c = si220_PDK.cells[cell_name]()
    expected = SI220_EXPECTED_PIN_NAMES[cell_name]
