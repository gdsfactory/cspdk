...
def xsection(func: Callable[..., CrossSection]) -> Callable[..., CrossSection]:
    ...
    ```python
        @xsection
        def strip_cband(width=TECH.width_cband, radius=TECH.radius_strip):
...
route_single = partial(gf.routing.route_single, cross_section="strip_cband")
route_bundle = partial(
    gf.routing.route_bundle, cross_section="strip_cband", sbend="bend_s"
)


route_bundle_rib = partial(
    route_bundle,
    cross_section="rib_cband",
)
...
if __name__ == "__main__":
    ...
    # print(strip_cband() is strip_cband())
    # print(strip_cband().name, strip_cband().name)
