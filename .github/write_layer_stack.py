...
_optional = [
    ...
    ("Si340", "cspdk.si340"),
    ("SiN200", "cspdk.sin200"),
    ("Ge-on-Si", "cspdk.ge_on_si"),
    ("Si Suspended", "cspdk.si_sus"),
]

for _label, _mod_path in _optional:
    try:
        _mod = __import__(_mod_path, fromlist=["PDK"])
        BANDS.append((_label, _mod.PDK))
    except ImportError:
        pass
...
def _compute_layout(layers, svg_w=550, svg_h=750):
    ...
    for layer in real:
        overlapping = [
            ol
            for ol in real
            if ol["zmin"] < layer["zmax"] - 1e-6 and ol["zmax"] > layer["zmin"] + 1e-6
        ]
...
def main():
    """Write docs/layer_stack.md with the interactive diagrams for every platform."""
    ...
    for band_label, pdk in BANDS:
        ...
        if ls is None:
            print(f"  Skipping {band_label}: no layer_stack")
            continue
